...
def run_trajectory(
    ...
    args:
        ...
        clusters_num:       # of clusters (k) for g.fit() as a list
   ...
   """
    ...
    g = cr.estimators.CFLARE(tmk)
    g.fit(k=clusters_num)

    log.info("[%s] predicting terminal states...", label)
    g.predict(cluster_key=cluster_key)
...
def main():
    ...
    log.info("tdtom subset: %d cells", adata_tdtom.n_obs)
    ...
    # tdtomato+ cells
    log.info("=== analysis 2: tdtomato+ cells ===")

    mask = adata.obs['tdtomato+'] == True
    adata_tdtom = adata[mask].copy()
    # remove neurons, heart, neural tube, pharyngeal region
    mask2 = (
        (adata_tdtom.obs['tdtom_0.6_cell_types'] == 'Heart') |
        (adata_tdtom.obs['tdtom_0.6_cell_types'] == 'Neural tube') |
        (adata_tdtom.obs['tdtom_0.6_cell_types'] == 'Pharyngeal region') |
        (adata_tdtom.obs['tdtom_0.6_cell_types'] == 'Neurons')
    )
    adata_tdtom = adata_tdtom[~mask2].copy()
    log.info("tdtom subset: %d cells", adata_tdtom.n_obs)

    try_plot(
        lambda: gpd.GeoDataFrame(adata_tdtom.obs, geometry="geometry").plot(
            column="tdtom_0.6_cell_types",
            cmap="tab20",
            ax=plt.subplots(figsize=(10, 8))[1],
            legend=True,
            legend_kwds={"loc": "upper left", "bbox_to_anchor": (1, 1)},
        ...
        PLOT_DIR_TDTOM,
        "tdtom_spatial_cell_types.png",
    )

    # recompute pca, harmony, neighbors, umap on the subset
    log.info("recomputing pca + harmony + neighbors + umap for tdtom subset...")
    sc.pp.pca(adata_tdtom)
    sc.pp.neighbors(adata_tdtom, use_rep="X_pca")
    sc.tl.umap(adata_tdtom, random_state=0)

    try_plot(
        lambda: sc.pl.umap(
            adata_tdtom, color=["sample", "timepoints"], show=False
        ),
        PLOT_DIR_TDTOM,
        "tdtom_umap_sample_timepoints.png",
    )

    driver_clusters_tdtom = [
        "Mesenchyme including BA",
        "Dermal bone",
    ...
    adata_tdtom, tmk_tdtom, g_tdtom, _ = run_trajectory(
        adata=adata_tdtom,
        cluster_key="tdtom_0.6_cell_types",
        ba_lineage="Mesenchyme including BA",
        driver_clusters=driver_clusters_tdtom,
        label="tdtom",
        out_dir=OUT_DIR_TDTOM,
        plot_dir=PLOT_DIR_TDTOM,
        clusters_num=8,
    )

    log.info("done. outputs in: %s", OUT_DIR_TDTOM)
